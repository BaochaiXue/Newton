#!/usr/bin/env python3
"""Controller-driven zebra over one-way MPM sand.
...
- run the zebra with SolverSemiImplicit
- write controller positions every substep
- expose the zebra to MPM only through a moving proxy mesh
- do not feed MPM impulses back into the zebra

The one-way coupling idea follows Newton's `example_mpm_anymal.py`, but the
collider here is a deforming mesh proxy because the zebra is a spring-mass
object, not a rigid-body articulation.
"""
...
def _build_controller_driver(ir: dict[str, np.ndarray], device: str) -> ControllerDriver:
    ...
    if ctrl_traj.ndim != 3 or ctrl_traj.shape[1] != ctrl_idx.size or ctrl_traj.shape[2] != 3:
        raise ValueError(f"Unexpected controller_traj shape {ctrl_traj.shape} for controller count {ctrl_idx.size}")
...
def _build_soft_model(
    ...
    spec = SoftSpec(
        ...
        color_rgb=(0.96, 0.97, 0.99),
    )
    bounds_lo = np.asarray(args.sand_bounds_lo, dtype=np.float32)
    bounds_hi = np.asarray(args.sand_bounds_hi, dtype=np.float32)
    meta = DemoMeta(
        soft_spec=spec,
        ...
        scene_label="Scene: controller-driven zebra + one-way MPM sand",
    )
...
def _build_proxy(
    ...
    proxy_points = np.sum(obj_pts[idx.astype(np.int32)] * weights[:, :, None], axis=1).astype(np.float32, copy=False)
    proxy_vel = np.sum(qd0[global_ids] * weights[:, :, None], axis=1).astype(np.float32, copy=False)
    ...
    auto_thickness = max(float(np.mean(spec.render_point_radius)), 0.75 * float(args.voxel_size))
    thickness = float(args.proxy_thickness) if args.proxy_thickness is not None else auto_thickness
    return SoftProxy(
...
def _build_sand_system(proxy: SoftProxy, args: argparse.Namespace, device: str) -> SandSystem:
...
def _update_proxy_from_soft_state(state: newton.State, proxy: SoftProxy, frame_dt: float) -> None:
...
def simulate(
    ...
    for frame in range(1, max_frames):
        ...
        if frame == 1 or frame % 10 == 0 or frame + 1 == max_frames:
            zebra_np = soft_hist[-1][: spec.particle_count]
            sand_np = sand_hist[-1]
            print(
                f"  frame {frame + 1}/{max_frames}"
                f"  zebra_z=[{zebra_np[:,2].min():.3f},{zebra_np[:,2].max():.3f}]"
...
def render_mp4(
    *,
    ...
    try:
        ...
        zebra_points_wp = wp.empty(zebra_count, dtype=wp.vec3, device=device)
        zebra_radii_wp = wp.array(meta.soft_spec.render_point_radius.astype(np.float32), dtype=wp.float32, device=device)
        zebra_colors_wp = wp.array(
            np.tile(np.asarray(meta.soft_spec.color_rgb, dtype=np.float32), (zebra_count, 1)),
            dtype=wp.vec3,
            device=device,
        )
        ...
        for frame_idx in range(n_frames):
            ...
            viewer.log_points("/demo/zebra", zebra_points_wp, zebra_radii_wp, zebra_colors_wp, hidden=False)
            ...
            if bool(args.render_soft_springs) and meta.soft_spec.render_edge_global.size > 0:
                ...
                viewer.log_lines("/demo/zebra_springs", None, None, None)
...
def _write_outputs(
    ...
    summary = {
        "experiment": "zebra_mpm_sand_one_way",
...
def main() -> int:
    ...
    print("Simulating controller-driven zebra...", flush=True)
    ...
    if not bool(args.skip_render):
        ...
        render_mp4(model=model, sim_data=sim_data, meta=meta, args=args, device=device, out_mp4=out_mp4)
