...
def build_model(ir_obj: dict[str, Any], args: argparse.Namespace, device: str):
    """Compatibility wrapper for the canonical cloth+bunny model builder."""

    from .offline import build_model as _build_model

    return _build_model(ir_obj, args, device)


def default_cloth_ir() -> Path:
    return _default_cloth_ir()


def validate_scaling_args(args: argparse.Namespace) -> None:
    _validate_scaling_args(args)


def copy_object_only_ir(ir: dict[str, np.ndarray], args: argparse.Namespace) -> dict[str, Any]:
    return _copy_object_only_ir(ir, args)


def maybe_autoset_mass_spring_scale(
    args: argparse.Namespace,
    raw_ir: dict[str, np.ndarray],
    *,
    target_total_mass: float,
) -> None:
    _maybe_autoset_mass_spring_scale(args, raw_ir, target_total_mass=target_total_mass)


def mass_tag(value: float) -> str:
    return _mass_tag(value)
...
__all__ = [
    ...
    "_default_cloth_ir",
    "_effective_object_mass_array",
    "_effective_spring_scales",
    "_mass_ratio_scale",
    "_mass_tag",
    "_maybe_autoset_mass_spring_scale",
    "_reconstruct_cloth_surface_mesh",
    ...
    "load_ir",
    "mass_tag",
    "maybe_autoset_mass_spring_scale",
    "quat_to_rotmat",
    "validate_scaling_args",
]
